...
class GameScene2Phone: SKScene {
    var elapsedTime = 0
    var speedTimer: Timer?
    let pixelsPerMeter: CGFloat = 100
    let inputWidth: Int = 100
    ...
    override func didMove(to view: SKView) {
        ...
        errorTextLabel.position = CGPoint(x: frame.midX, y: frame.midY - 20)
        ...
        terrain.position = .init(x: 250, y: 110)
        addChild(terrain)
        ...
        durationLabel.position = CGPoint(x: 140, y: 265)
        addChild(durationLabel)
        
        forceInputField = UITextField(frame: CGRect(x: 50, y: 230, width: inputWidth, height: inputHeight))
        forceInputField.textAlignment = .center
        forceInputField.attributedPlaceholder = NSAttributedString(
            string: "Force (N)",
        ...
        forceInputField.textColor = .white
        forceInputField.layer.cornerRadius = 5
        forceInputField.layer.borderColor = .init(red: 0, green: 0, blue: 0, alpha: 1)
        forceInputField.layer.borderWidth = 2
        
        view.addSubview(forceInputField)
        
        massInputField = UITextField(frame: CGRect(x: 220, y: 230, width: inputWidth, height: inputHeight))
        massInputField.textAlignment = .center
        massInputField.attributedPlaceholder = NSAttributedString(
            string: "Mass (kg)",
            attributes: [NSAttributedString.Key.foregroundColor: UIColor.white]
        )
        ...
        massInputField.textColor = .white
        massInputField.layer.cornerRadius = 5
        massInputField.layer.borderColor = .init(red: 0, green: 0, blue: 0, alpha: 1)
        ...
        moveButton.frame = CGRect(x: frame.midX-75, y: 280, width: 150, height: 30)
